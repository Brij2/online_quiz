...
const DisplayQue = () => {
  ...
  useEffect(() => {
    ...
    Axios.get(`https://quiz-server-ab0n.onrender.com/get/quiz/${id}`).then((response) => {
      if (response.data.length === 0) {
        alert("No Quiz with such ID exists!");
        navigate("/")
      }
      else {
        Axios.get(`https://quiz-server-ab0n.onrender.com/result/${prn}/${id}`).then((res) => {
          if (res.data.length === 0) {
            const quiz_id = response.data[0]['quiz_id']

            Axios.post("https://quiz-server-ab0n.onrender.com/result/init", {
              prn: prn,
              quizID: quiz_id
            }).then((resp) => {
              fetch("http://ip-api.com/json/?fields=country,regionName,city,isp,query,lat,lon")
                ...
                .then((res) => {
                  ...
                  Axios.post("https://quiz-server-ab0n.onrender.com/attempts/init", {
                    prn: prn,
                    quiz_id: quiz_id,
                    ip_addr: ip_addr,
                    country: country,
  ...
  const submitQuiz = () => {
    ...
    Axios.post(`https://quiz-server-ab0n.onrender.com/quiz/submit`, {
      prn: prn,
      quiz_id: quizID,
